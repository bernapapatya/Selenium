...
public class Fin_Class {
	   ...
	   static WebDriver driver; 
		...
		System.out.println("Solutions Sayfasý Açýldý");
	    
		List<WebElement> rows = driver.findElements(By.className("title"));
		System.out.println("Seçilen toplam satýr: " + rows.size());
		
        //iterator, listelerden geçiþ yapmasýný saðlayan bir nesnedir.
		//Ýterator kullanarak tüm unsurlarý yineleyecegiz 
		Iterator<WebElement> iter = rows.iterator();
        //Bu listenin bir elemaný olup olmadýðýný kontrol eder
			while (iter.hasNext())  {
		...
		//driver.close();
		//System.out.println("Tarayýcý baþarýyla kapatýldý");
